...
export class PokemonsService {
     ...
     * @param pageNumber
     */
    getPokemons(pageNumber: number = 0): Observable<NamedAPIResourceDto[]>{
        this.logger.log('Returning ' + LIMIT + ' pokemon from page : ' + pageNumber);

        const urlToCall = URL_API_POKEMON + '?limit=' + LIMIT + '&offset=' + (pageNumber * LIMIT);
        return this.httpService.get<NamedAPIResourceListDto>(urlToCall)
            .pipe(map((response: AxiosResponse<NamedAPIResourceListDto>) => response.data))
    ...
    getPokemon(pokemonName : string) : Observable<PokemonDto> {
        ...
        return this.httpService.get(urlToCall)
